...
def _post(url, **kwargs):
    return requests.post(url, timeout=kwargs.pop("timeout", _TIMEOUT), **kwargs)


def _get(url, **kwargs):
    return requests.get(url, timeout=kwargs.pop("timeout", _TIMEOUT), **kwargs)
...
def test_every_provider_happy_path(server):
    ...
    r = _post(server.base_url + path, headers=headers, json=body)
...
class TestOpenAI:
    ...
    GOOD: ClassVar[dict] = {"Authorization": "Bearer sk-fake"}
...
class TestAnthropic:
    URL = "/v1/messages"
    GOOD: ClassVar[dict] = {"x-api-key": "sk-ant-fake", "anthropic-version": "2023-06-01"}
    BODY: ClassVar[dict] = {"model": "claude-3-7-sonnet-latest", "max_tokens": 50,
            "messages": [{"role": "user", "content": "hi"}]}
...
class TestGemini:
    ...
    BODY: ClassVar[dict] = {"contents": [{"role": "user", "parts": [{"text": "hi"}]}]}

    def test_native_happy_path(self, server):
        r = _post(server.base_url + self.GEN, headers={"x-goog-api-key": "AIzaX"}, json=self.BODY)
        assert r.status_code == 200
        assert r.json()["candidates"][0]["content"]["parts"][0]["text"]

    def test_query_param_key_auth(self, server):
        r = _post(server.base_url + self.GEN + "?key=AIzaX", json=self.BODY)
        assert r.status_code == 200

    def test_missing_key_403(self, server):
        r = _post(server.base_url + self.GEN, json=self.BODY)
        assert r.status_code == 403

    def test_bad_key_is_400_not_401(self, server):
        r = _post(server.base_url + self.GEN, headers={"x-goog-api-key": "badkey"}, json=self.BODY)
        assert r.status_code == 400
    ...
    def test_assistant_role_rejected(self, server):
        ...
        r = _post(server.base_url + self.GEN, headers={"x-goog-api-key": "AIzaX"}, json=body)
        assert r.status_code == 400

    def test_bad_model_404(self, server):
        url = server.base_url + "/v1beta/models/gemini-9.9-nope:generateContent"
        r = _post(url, headers={"x-goog-api-key": "AIzaX"}, json=self.BODY)
    ...
    def test_unknown_top_level_field_400(self, server):
        ...
        r = _post(server.base_url + self.GEN,
    ...
    def test_openai_shim_bearer(self, server):
        r = _post(server.base_url + "/v1beta/openai/chat/completions",
...
class TestDeepSeek:
    ...
    def test_balance_endpoint(self, server):
        r = _get(server.base_url + "/user/balance", headers=self.GOOD)
...
class TestMistral:
    ...
    BODY: ClassVar[dict] = {"model": "mistral-large-latest", "messages": [{"role": "user", "content": "hi"}]}
...
class TestOllama:
    def test_native_non_stream(self, server):
        r = _post(server.base_url + "/api/chat",
...
def test_scripting_overrides_real_logic(server):
    ...
    r2 = _post(server.base_url + "/v1/chat/completions",
                       headers={"Authorization": "Bearer sk-x"},
                       json={"model": "gpt-4o", "messages": [{"role": "user", "content": "hi"}]})
